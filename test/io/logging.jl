...
function _capture_stdout(f::Function)
 ...
 @testset "Logging" begin
     ...
     @testset "Integration detail routing honors file, console, and off modes" begin
        temp_file = mktempdir()
        ...
        try
            ...
            console_text = _capture_stdout() do
                terra.emit!(terra.RUN_LOG, config_console.runtime,
                            terra.IntegrationDetailEntry("console detail"))
            end
            @test occursin("console detail", console_text)
            @test !isfile(terra.run_log_path(config_console.runtime))

            config_off = _logging_test_config(temp_off; console_mode = :quiet,
                                              integration_detail_mode = :off)
            console_off = _capture_stdout() do
                terra.emit!(terra.RUN_LOG, config_off.runtime,
                            terra.IntegrationDetailEntry("ignored detail"))
            end
            @test isempty(console_off)
            @test !isfile(terra.run_log_path(config_off.runtime))
        finally
