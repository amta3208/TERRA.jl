    ...
    return with_active_runtime_for_logging(runtime; restore = restore_runtime) do
        try
            ...
            emit!(RUN_LOG, runtime,
                  EventEntry(:info, "Initializing TERRA";
            ...
            emit!(RUN_LOG, runtime,
                  EventEntry(:debug, "TERRA input files generated";
                             console = :never,
                             :case_path => case_path))
            ...
            emit!(RUN_LOG, runtime,
                  EventEntry(:info, "TERRA initialized successfully";
                             console = lifecycle_console,
            ...
            try
                ...
                emit!(RUN_LOG, runtime,
                      EventEntry(:debug, "TERRA runtime flags";
                                 console = :never,
            ...
            emit!(RUN_LOG, runtime,
                  ExceptionEntry(:error, "Failed to initialize TERRA", e;
                                 console = :minimal))
            rethrow(e)
        end
    end
...
function finalize_terra()
    ...
    return with_active_runtime_for_logging(runtime; restore = nothing) do
        try
            emit!(RUN_LOG, runtime,
                  EventEntry(:info, "Finalizing TERRA"; console = :minimal))
            finalize_api_wrapper()
            close_terra_library()
            emit!(RUN_LOG, runtime,
                  EventEntry(:info, "TERRA finalized successfully";
                             console = :minimal))
        catch e
            emit!(RUN_LOG, runtime,
                  ExceptionEntry(:error, "Error during TERRA finalization", e;
                                 console = :minimal))
            rethrow(e)
        end
    end
