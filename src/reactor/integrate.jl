...
struct ReactorODEContext
    ...
    teex_const::Float64
    teex_const_vec::Vector{Float64}
    work_u::Vector{Float64}
    work_y::Vector{Float64}
    work_rho_sp::Vector{Float64}
    work_rho_ex::Union{Nothing, Matrix{Float64}}
    sources::PreparedSources
    progress_reporter::Union{Nothing, FractionProgressReporter}
end
...
function native_ramp_callback(initial_dt; understep_ratio = inv(128), history_steps = 5)
    ...
    return DiscreteCallback(native_ramp_condition, affect!;
...
function integration_progress_affect!(integrator)
    ...
    fraction = reporter.tlim > 0.0 ?
               clamp(Float64(integrator.t) / reporter.tlim, 0.0, 1.0) : 1.0
...
_failed_reactor_result(message::AbstractString) = ReactorResult(; t = Float64[],
                                                                frames = ReactorFrame[],
                                                                source_terms = nothing,
                                                                success = false,
                                                                message = String(message))

function _failed_reactor_result(config::Config,
    ...
    fallback_frame = ReactorFrame(; t = 0.0,
...
function _integrate_0d_system(config::Config,
    ...
    context = ReactorODEContext(layout,
                                config,
                                molecular_weights,
                                species_names,
                                gas_constants,
                                initial_state.teex_const,
                                teex_const_vec,
                                work_u,
                                work_y,
                                work_rho_sp,
    ...
    prob = ODEProblem(terra_ode_system!, u0, tspan, context)
    ...
    try
        ...
        if config.runtime.unit_system == :SI
            ...
            for i in axes(species_densities, 2)
                species_densities_si[:, i] = convert_density_cgs_to_si(species_densities[:, i])
            end
        ...
        for i in 1:n_times
            ...
            frames[i] = ReactorFrame(; t = time_points[i],
        ...
        return ReactorResult(; t = time_points,
                             frames = frames,
                             source_terms = nothing,
                             success = success,
                             message = message,
