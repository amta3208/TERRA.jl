@inline function _is_electron_species(name::AbstractString, molecular_weight::Real)
    ...
    species in ("E", "E-", "E+", "ELEC", "ELECTRON") && return true
...
"""
function enthalpy_from_energy(rho_etot::Float64,
...
@inline function _compact_nonnegative_ok(u::AbstractVector{<:Real}, layout::ApiLayout)
    any(@view(u[layout.vib_range]) .< 0.0) && return false
    any(@view(u[layout.elec_range]) .< 0.0) && return false
    any(@view(u[layout.sp_range]) .< 0.0) && return false
    u[layout.idx_etot] < 0.0 && return false
    layout.idx_eeex != 0 && u[layout.idx_eeex] < 0.0 && return false
    layout.idx_erot != 0 && u[layout.idx_erot] < 0.0 && return false
    layout.idx_evib != 0 && u[layout.idx_evib] < 0.0 && return false
    return true
end
...
@inline function _compact_project_density_nonnegative!(u_work::Vector{Float64},
    ...
    (!(rho_target > 0.0) || !isfinite(rho_target)) && return false
    (!(rho_clamped > 0.0) || !isfinite(rho_clamped)) && return false

    scale = rho_target / rho_clamped
    (!(isfinite(scale)) || scale <= 0.0) && return false
...
ODE system that calls the Fortran `rhs_api` directly.
"""
...
function _format_terra_integration_output(t::Float64,
    ...
    println(out, @sprintf(" time       = % .2E mu-s ", t * 1e6))
    println(out, @sprintf(" dt         = % .2E mu-s", dt * 1e6))
