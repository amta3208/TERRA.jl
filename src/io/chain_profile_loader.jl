...
function _require_number_field(container::AbstractDict, key::AbstractString, context::AbstractString)
    haskey(container, key) || throw(ArgumentError("Missing `$key` in $context."))
    value = container[key]
...
function _require_string_array_field(container::AbstractDict, key::AbstractString, context::AbstractString)
    ...
    for (i, value) in pairs(values)
        value isa AbstractString || throw(ArgumentError(
            "Expected string values in `$context.$key`; got $(typeof(value)) at index $i."
        ))
...
function _build_chain_profile_inlet(raw::AbstractDict)
    ...
    source_compact_index isa Integer || (
        ...
        "Expected `source_compact_index` in chain profile inlet to be an integer."
    ))

    inlet_composition = ChainProfileInletComposition(;
        species = _require_string_array_field(composition, "species", "inlet.composition"),
        mole_fractions = _require_float_array_field(composition, "mole_fractions",
            "inlet.composition"),
        total_number_density_m3 = _require_number_field(composition,
            "total_number_density_m3", "inlet.composition"),
    )
    inlet_thermal = ReactorThermalState(;
        Tt = _require_number_field(thermal, "Tt_K", "inlet.thermal"),
        Tv = _require_number_field(thermal, "Tv_K", "inlet.thermal"),
        Tee = _require_number_field(thermal, "Tee_K", "inlet.thermal"),
        Te = _require_number_field(thermal, "Te_K", "inlet.thermal"),
    )
...
function _validate_chain_profile_schema(raw::AbstractDict)
    ...
    for key in ("Tt_K", "Tv_K", "Tee_K", "Te_K")
        haskey(inlet_thermal, key) || throw(ArgumentError(
            "Missing required inlet thermal field `$key`."
        ))
    end
    haskey(inlet, "source_compact_index") || throw(ArgumentError(
        "Missing required inlet field `source_compact_index`."
    ))
