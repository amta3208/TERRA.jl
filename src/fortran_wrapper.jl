...
Optional inputs:
- `energy_cache`: in/out scalar warm-start for the internal enthalpy→energy inversion
...
function calculate_rhs_api_isothermal_teex_wrapper!(du::Vector{Float64}, u::Vector{Float64},
    ...
    GC.@preserve u du energy_cache tex begin
        u_ptr = Base.unsafe_convert(Ptr{Float64}, u)
        du_ptr = Base.unsafe_convert(Ptr{Float64}, du)
        energy_ptr = energy_cache === nothing ? C_NULL :
                     Ptr{Cvoid}(Base.unsafe_convert(Ptr{Float64}, energy_cache))
        ...
        ccall((:calculate_rhs_api_isothermal_teex, get_terra_lib_path()), Cvoid,
            ...
            neq32, u_ptr, du_ptr, teex_const, energy_ptr, tex_ptr)
    end
